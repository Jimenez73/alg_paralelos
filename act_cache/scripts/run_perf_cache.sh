...
RESULTS_DIR="results"
CSV="$RESULTS_DIR/perf_stats_cache.csv"

mkdir -p "$RESULTS_DIR"

# Preparamos el encabezado del archivo CSV
if [[ ! -f "$CSV" ]]; then
    echo "kernel,n,bindir,instructions,cycles,cache-references,cache-misses,L1-dcache-loads,L1-dcache-load-misses,LLC-loads,LLC-load-misses" > "$CSV"
fi
...
for KERNEL in "$BINDIR"/*; do
    ...
    for N in "${SIZES[@]}"; do
        ...
        perf stat -e instructions,cycles,cache-references,cache-misses,L1-dcache-loads,L1-dcache-load-misses,LLC-loads,LLC-load-misses "$KERNEL" "$N" > /dev/null 2> "$TMP_PERF"
        ...
        L1_LOADS=$(grep "L1-dcache-loads" "$TMP_PERF" | awk '{print $1}' | tr -d '.' | tr -d ',')
        L1_MISS=$(grep "L1-dcache-load-misses" "$TMP_PERF" | awk '{print $1}' | tr -d '.' | tr -d ',')
        L3_LOADS=$(grep "LLC-loads" "$TMP_PERF" | awk '{print $1}' | tr -d '.' | tr -d ',')
        L3_MISS=$(grep "LLC-load-misses" "$TMP_PERF" | awk '{print $1}' | tr -d '.' | tr -d ',')
        
        # Guardamos en el CSV
        echo "$NAME,$N,$BINDIR,$INS,$CYC,$REF,$MISS,$L1_LOADS,$L1_MISS,$L3_LOADS,$L3_MISS" >> "$CSV"
        echo "Listo"
