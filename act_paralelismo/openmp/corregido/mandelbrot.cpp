...
int main() {
    ...
    // Se recomienda omp parallel for con schedule(dynamic) por el desbalance de carga
    #pragma omp parallel for schedule(static) num_threads(4)
    for (int row = 0; row < H; row++) {
        // Las variables se declaran localmente para que cada hilo tenga su propia copia
        double ci = YMIN + (YMAX - YMIN) * row / (H - 1);
        
        for (int col = 0; col < W; col++) {
            double cr   = XMIN + (XMAX - XMIN) * col / (W - 1);
            double zr   = 0.0;
            double zi   = 0.0;
            int iter = 0;
            ...
            // img no tiene conflictos de escritura porque cada hilo escribe en índices únicos
